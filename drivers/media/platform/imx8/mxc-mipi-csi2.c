...
static void mxc_mipi_csi2_csr_config(struct mxc_mipi_csi2_dev *csi2dev)
{
	...
	/* All datatypes enabled */
	val = 0;
	writel(val, csi2dev->csr_regs + CSI2SS_DATA_TYPE_DISABLE);
...
static int mipi_csi2_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (sen_sd->internal_ops && sen_sd->internal_ops->close) {
		return sen_sd->internal_ops->close(sen_sd, fh);
	}
...
static int mipi_csi2_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (sen_sd->internal_ops && sen_sd->internal_ops->open) {
		return sen_sd->internal_ops->open(sen_sd, fh);
	}
...
static int mipi_csi2_probe(struct platform_device *pdev)
{
	...
	csi2dev->sd.internal_ops = &mipi_csi2_sd_internal_ops;
